...
# Just like the create_environement,this will exit immediately if a command exits.
set -e

# I prompt the user for the name they used when creating the environment. This is very important to locate the correct 'submission_reminder_{yourName}' directory.
read -p "Enter the name you used when creating the application environment (e.g., jacob): " ENV_USER_NAME

# This assumes that the  script is run from the same directory where 'create_environment.sh' created the supposed  app folder.
MAIN_APP_DIR="submission_reminder_${ENV_USER_NAME}"

# Check if the main application directory exists. And if it does not , inform the user and exit.
...
# The config.env file is inside the 'config' subdirectory that is in the main app directory.
CONFIG_FILE="${MAIN_APP_DIR}/config/config.env"

# Now confirm if the config.env file actually exists at the path.
